...
echo "Checking VPS"
clear
source /var/lib/kuyax/ipvps.conf
if [[ "$IP" = "" ]]; then
...
xrayvless1="vless://${uuid}@${domain}:$tls?path=/kuyaxvl/&security=tls&encryption=none&type=ws#${user}"
xrayvless2="vless://${uuid}@${domain}:$nontls?path=/kuyaxvl/&encryption=none&type=ws#${user}"
